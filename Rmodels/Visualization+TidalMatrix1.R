rm(list=ls(all=TRUE)) #https://www.zoology.ubc.ca/~schluter/R/meta/
...
simulation = function(replicas){
  ...
  for (k in 1:replicas){ #initiate large loop
    ...
    #spplot(obj=yy1[1], col.regions=colorRampPalette(c('black', 'gray80','red')))
    #############################################################
    #acf(yy$sim1,main='Autocorrelation function') # correlated
    #acf(yy1$sim1,main='Autocorrelation function') # random
    ################################################################
    ...
    #XEnc <- sapply(aux, function(x) {
    #  matrix(1, ncol = ncol, nrow = nrow)
    #}, simplify = FALSE) # biological matrix = Enclosures
    ...
    #XEnc[[1]] = round(abs(vecTopo1 + rnorm(ncol*nrow, 0, 2))) # adding individuals to the starting point
    ...
    for (t in 1: (span - 1)){ # number of matrices
       for (i in 1: (ncol * nrow)){
          X[[t + 1]][i] = X[[t]][i] * exp(rEnc * Ua[[t]][[i]] * (1 - (X[[t]][i] / K))) + Z[[t]][i] #solo Ulva
          #XEnc[[t + 1]][i] = XEnc[[t]][i] * exp(rEnc * Ua[[t]][[i]] * (1 - (XEnc[[t]][i] / K))) + Z[[t]][i]
          XOpen[[t + 1]][i] = XOpen[[t]][i] * exp(rEnc * Ug[[t]][[i]] * (1 - (XOpen[[t]][i] / K))) + Z[[t]][i] 
          #XOpen[[t + 1]][i] = XEnc[[t]][i] * 0.95 + round(abs(rnorm(1, 3, 2))) #98% exclusion + 5%error
          } # 
    }
...
Bob = unlist(Res_simul[[2]]$D) 
length(Bob)
length(seq(1, 364, 1))
Roby = round(Bob, 3)

par(mar = rep(2, 4))
plot(seq(1, 364, 1), Roby) #, type = "l",col=4, xlab='time',ylab='Fractal D', pch=1) #, solo pa cachar!
names(simulation)
...
###############SAVE FILES############ WARNING
setwd("C:/Users/localadmin_eliediaz/Desktop") #Users\localadmin_eliediaz\Desktop
dput(X, file = "fractallowR26June.txt")
X <- dget(file = 'X.txt') 
