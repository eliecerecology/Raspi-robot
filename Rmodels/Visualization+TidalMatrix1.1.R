...
setwd("/home/eliecer/Desktop") # Linux
...
simulation_Nat = function(replicas){
  ...
  for (k in 1:replicas){ #initiate meta-loop
    ...
    #### META-LOOP!
    Result_Nat[[k]]    = cbind(D, Dx,Var_XER, Var_X, mean_XER, mean_X) 
...
} #######END Natural SIMULATION########
  
  
#FUNCTION SIMULATION (SIMUlATION WEATHER)
...
setwd("C:/Users/localadmin_eliediaz/Dropbox/DATA_CLIMATE")#New folder
...
#####NON-RANDOM
######1. Natural
D_Nat = list();  Dx_Nat = list();  Var_XER_Nat = list(); Var_X_Nat = list()
mean_XER_Nat = list(); mean_X_Nat = list()
####2. simulation
...
for (l in 1:k){
      D_Nat[[l]] = unlist(Res_simul_Nat[[l]]$D)
      Dx_Nat[[l]] = unlist(Res_simul_Nat[[l]]$Dx)
      Var_XER_Nat[[l]] = unlist(Res_simul_Nat[[l]]$Var_XER)
      Var_X_Nat[[l]] = unlist(Res_simul_Nat[[l]]$Var_X)
      mean_XER_Nat[[l]] = unlist(Res_simul_Nat[[l]]$mean_XER)
      mean_X_Nat[[l]] = unlist(Res_simul_Nat[[l]]$mean_X)
...
lines(seq(1,length(D_Nat[[s]]), 1), D_Nat[[s]], type = "l",col="green", xlab='time',ylab='Fractal D', pch=1)
...
lines(seq(1,length(Dx_Nat[[s]]), 1), Dx_Nat[[s]], type = "l",  col="green")
...
lines(seq(1,length(Var_XER_Nat[[s]]), 1), Var_XER_Nat[[s]], type = "l",  col="green")
...
lines(seq(1,length(Var_XER_Nat[[s]]), 1), Var_X_Nat[[s]], type = "l",  col="green")
...
lines(seq(1,length(mean_XER_Nat[[s]]), 1), mean_XER_Nat[[s]], type = "l",  col="green")
...
lines(seq(1,length(mean_XER_Nat[[s]]), 1), mean_X_Nat[[s]], type = "l",  col="green")
Sys.sleep(1)
}
