...
lags =  30 #length(s$red)/2 #NUMBER OF LAGS Allowed to Use (half of the transect)
var1 =  s$ulva
var2 <- s$biomass
...
cross_semivariogram <- function(var1, var2, permutations) {
    ...
    for(j in 1: lags){ #number of matrix
      for(i in 1: (n)-1){ # first lag, n = 59
        ...
        b[[j]] <- sum(o[[j]][1:(n)], na.rm=TRUE)
        cross_s[[j]] <- (b[[j]]/ (2*(n-j+1)) ) # +1 because n is UNEVEN = 59, if 60 use "n-j"
      }
    }
    ...
    for (k in 1:permut){
      for(j in 1: lags){ #number of matrix
        for(i in 1: (n)-1){ # first lag, n = 59
          ...
          b[[j]] <- sum(o[[j]][1:(n)], na.rm=TRUE)
          c_list[[k]][j] <- (b[[j]]/ (2*(n-j+1)) ) # +1 because n is UNEVEN = 59, if 60 use "n-j"
        }
      }
    }
    ...
    list(P_values = pvalues, P_values_small_than_0.05 = which_ones)

}
cross_semivariogram(var1, var2, 10) # calling function
